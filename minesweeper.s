...
# This program was written by Maximilian Helm (z5314713)
# 22.10 - 31.10
#
...
reveal_grid__body:
        ...
        la      $t2, grid                       # load the address of the array into $t2

reveal_grid_loop:                               # loop through the grid
        bge     $t0, N_CELLS, reveal_grid__epilogue
                                                # leave the loop if we moved through the whole grid         

        # grid[row][col] |= IS_RVLD_MASK                                
        add     $t3, $t2, $t0                   # calculate the address of the current index and save it in $t3
        lb      $t4, ($t3)                      # load the cell's value into $t4
        or      $t4, $t4, IS_RVLD_MASK          # make it revealed by using the "or" bit operator 
        sb      $t4, ($t3)                      # store the revealed value at that particular array index
...
place_bombs__body:
        ...
        li      $s2, 0                          # set the counter looping through the number of bombs to 0
                                                # use a saved register, so that the counter does not get overwritten
        move    $s0, $a0                        # save the arguments passed into that function in saved variables
        move    $s1, $a1
        lw      $s3, total_bombs                # load the word saved in total_bombs into $s3

place_bombs_loop:
        # for (int i = 0; i < total_bombs; i++)
        bge     $s2, $s3, place_bombs__epilogue # leave the loop if we placed all bombs          

        move    $a0, $s0                        # restore the argument variables before every function call
        move    $a1, $s1

        jal     place_single_bomb               # call the place_single_bomb function
...
mark_cell__body:
        ...
        li      $t0, N_COLS                     # load the horizontal dimension (x coordinate) of the grid 
                                                # into $t0, to be able to get the address of grid[row][col];
        
        la      $t2, grid                       # load address of the grid into $t2

        # get the address of target field       # position = (row_size * i + j) * sizeof(element)
        mul     $t3, $t0, $a0                   # row_size * row ($a0); save result in $t3
        add     $t3, $t3, $a1                   # (address of start of row $a0) + skip addresses until column $a1
        add     $t2, $t2, $t3                   # address of grid[row][col] stored in $t2


        # assess of field is revealed
        # if (grid[row][col] & IS_RVLD_MASK)
        lb      $t5, ($t2)                      # store the value of target field into $t5
        
        and     $t4, $t5, IS_RVLD_MASK          # check if the cell is revealed by doing a "and" bit operation
        beqz    $t4, mark_cell_unmark_cell      # and comparing the result to 0; if the value is 0, then go the
                                                # next part of the function, in which we unmark the cell

        # assess if debug mode is on
        lw      $t3, debug_mode                 # save debug mode in variable $t3
        beq     $t3, TRUE, mark_cell__epilogue  # compare 1 with $t3, which is the debug variable value; 
                                                # if the debug value is True, then skip to the epilogue
                
        la      $a0, mark_cell_str0             # printf("Cannot mark a revealed cell.");
        li      $v0, 4
        syscall
        li      $a0, '\n'                       # printf("%c", '\n');
        li      $v0, 11
...
mark_cell_unmark_cell:
        # Unmark a marked cell
        # if (grid[row][col] & IS_MRKD_MASK)
        lw      $t1, bomb_count                 # save the number of bombs in $t1
        and     $t4, $t5, IS_MRKD_MASK          # check if the cell is marked by doing a "and" bit operation
        beqz    $t4, mark_cell_action           # and comparing the result to 0; if it is 0, then go to the next
                                                # function part in which we mark the cell

        # grid[row][col] &= ~IS_MRKD_MASK;
        li      $t6, IS_MRKD_MASK
        not     $t6, $t6
        and     $t5, $t5, $t6                   # unmark the cell by doing a "and" bit operation with the inverse of
                                                # IS_MRKD_MASK;
        addi    $t1, $t1, 1                     # increase bomb count by 1        

        j       mark_cell_sv_overwrttn_values   # jump to "save section" to skip the "else" statement

mark_cell_action:
        # Mark a cell
        # grid[row][col] |= IS_MRKD_MASK;
        or      $t5, $t5, IS_MRKD_MASK          # mark the cell by doing a "or" bit operation with the value of the cell
        addi    $t1, $t1, -1                    # decrease bomb count by 1



mark_cell_sv_overwrttn_values:
        # save the overwritten value of the field, as well as the cell count
        
        sb      $t5, ($t2)                      # save the overwritten value of the cell
        sw      $t1, bomb_count                 # save the overwritten bomb count
...
reveal_cell__prologue:
        addiu   $sp, $sp, -4                    # save all the save registers and $ra on the stack
        sw      $ra, 0($sp)
        addiu   $sp, $sp, -4
        sw      $s0, 0($sp)
        addiu   $sp, $sp, -4
...
reveal_cell__body:
        ...
        li      $t0, N_COLS                     # load the horizontal dimension (x coordinate) of the grid 
                                                # into $t0, to be able to get the address of grid[row][col] 
        la      $t1, grid                       # load address of the grid into $t1
        move    $s1, $a0                        # save "row" ($a0) in $s1 and "col" ($a1) in $s2, so we  
        move    $s2, $a1                        # can recall them when we need to call clear_surroundings
                                

        # get the address of target field       # position = (row_size * i + j) * sizeof(element)
        mul     $t3, $t0, $a0                   # row_size * row ($a0); save result in $t3
        add     $t3, $t3, $a1                   # (address of start of row $a0) + skip addresses until column $a1
        add     $s3, $t1, $t3                   # save address of grid[row][col] in $s3
        lb      $s0, ($s3)                      # load the value of grid[row][col] into $s0
        #or

        # if (grid[row][col] & IS_MRKD_MASK)
        and     $t7, $s0, IS_MRKD_MASK                  # check if the cell is marked by doing an "and" bit operation
        beqz    $t7, reveal_cell_check_if_revealed      # and compare the result to 0; if the value is 0, go to the 
                                                        # reveal check

        # check if debug mode is on
        lw      $t2, debug_mode                         # save debug mode in variable $t2
        beq     $t2, TRUE, reveal_cell__epilogue        # compare the debug variable value with 1 to assess if it is active;
                                                        # if the debug value is True, then skip to the epilogue

        la      $a0, reveal_cell_str0           # printf("Cannot mark a revealed cell.");
        li      $v0, 4
        syscall
        li      $a0, '\n'                       # printf("%c", '\n');
        li      $v0, 11
...
reveal_cell_check_if_revealed:
        
        # if (grid[row][col] & IS_RVLD_MASK)
        and     $t7, $s0, IS_RVLD_MASK          # check if the cell is revealed by doing an "and" bit operation
        beqz    $t7, reveal_cell_game_over      # and compare the result to 0; if the value is 0, then the cell 
                                                # is not revealed; go to the game_over part of the function

        # check if debug mode is on
        lw      $t2, debug_mode                 # save debug mode in variable $t2
        beq     $t2, TRUE, reveal_cell__epilogue
                                                # if the debug value is True, then skip to the epilogue

        la      $a0, reveal_cell_str1           # printf("Cell is already revealed.");
        li      $v0, 4
        syscall
        li      $a0, '\n'                       # printf("%c", '\n');
        li      $v0, 11
...
reveal_cell_game_over:
        # if (grid[row][col] & IS_BOMB_MASK)
        and     $t7, $s0, IS_BOMB_MASK          # check if the cell is a bomb by doing an "and" bit operation
        beqz    $t7, reveal_cell_revealing      # and compare the result to 0; if the value is 0, skip to next 
                                                # part of the function, in which we reveal the cell

        # game_state = LOSE
        li      $t4, 2                          # save 2 in $t4, because that is the value we want to overwrite the 
        sw      $t4, game_state                 # game status with


reveal_cell_revealing:

        # if ((grid[row][col] & VALUE_MASK) == 0)
        and     $t7, $s0, VALUE_MASK            # check if the cell's last 4 bits have value 0 by doing an 
        beqz    $t7, reveal_cell_revealing_if   # "and" bit operation and compare the result to 0;
                                                # go to the if statement if the value is 0

        j       reveal_cell_revealing_else      # otherwise go to else statement if the value is 1-15

reveal_cell_revealing_if:

        move    $a0, $s1                        # restore the argument $a0 and make it to value "row"
        move    $a1, $s2                        # restore the argument $a1 and make it to value "col"

        jal     clear_surroundings              # call clear_surroundings
        j       reveal_cell_final_if

reveal_cell_revealing_else:
        
        # grid[row][col] |= IS_RVLD_MASK;
        or      $s0, $s0, IS_RVLD_MASK          # reveal the cell by using an "or" bit operation with the 
                                                # cell's value and IS_RVLD_MASK
        sb      $s0, ($s3)

        # if (game_state != LOSE)
        lw      $t4, game_state                 # compare the game_state to the value of losing (2);
        beq     $t4, 2, reveal_cell_final_if    # if the game is in a losing state, jump to the last if statement
        

        lw      $t5, cells_left                 # load the cells count left into $t5, decrement it and save it
        addi    $t5, $t5, -1
        sw      $t5, cells_left


reveal_cell_final_if:
        
        # if (cells_left == 0)
        lw      $t5, cells_left                 # check if the number of left cells is 0, by loading the value 
        bne     $t5, 0, reveal_cell__epilogue   # and comparing it to 0; if it is not 0, jump to the epilogue

        li      $t4, 1                  
        sw      $t4, game_state                 # set the game_state equal to 1 (WIN)


reveal_cell__epilogue:
        lw      $s3, 0($sp)                     # load all save registers and $ra from the stack
        addiu   $sp, $sp, 4
        lw      $s2, 0($sp)
        addiu   $sp, $sp, 4
        lw      $s1, 0($sp)
...
clear_surroundings__prologue:
        addiu   $sp, $sp, -16                   # save the save registers and the return address on the stack pointer
        sw      $s0, 12($sp)
        sw      $s1, 8($sp)
        sw      $s2, 4($sp)
        sw      $ra, 0($sp)
...
clear_surroundings__body:
        ...
        move    $s1, $a0                        # save the arguments passed into the function, so we can reuse
        move    $s2, $a1                        # them during our recursive function calls

        # if (row < 0 || row >= N_ROWS || col < 0 || col >= N_COLS) return;
        li      $t0, N_ROWS                     # save the number of COLS and ROWS into a variable to compare
        li      $t1, N_COLS                     # it in the if statement
        blt     $a0, 0, clear_surroundings__epilogue    # if row < 0 return
        bge     $a0, $t0, clear_surroundings__epilogue  # if row >= N_ROWS return
        blt     $a1, 0, clear_surroundings__epilogue    # if col < 0 return
        bge     $a1, $t1, clear_surroundings__epilogue  # if col >= N_COLS return


        # if (grid[row][col] & IS_RVLD_MASK) return;
        # find the address and value of grid[row][col]
        la      $t3, grid
        
        mul     $t2, $t1, $a0                   # row_size * row ($a0); save result in $t2
        add     $t2, $t2, $a1                   # (address of start of row $a0) + skip addresses until column $a1
        add     $t2, $t3, $t2                   # address of grid[row][col] stored in $t2
        lb      $s0, ($t2)                      # save the value of grid[row][col] in $s0

        and     $t7, $s0, IS_RVLD_MASK          # check if the cell is revealed by doing an "and" bit operation
        beqz    $t7, clear_surroundings_reveal  # and compare the result to 0; if the value is 0, then the cell 
                                                # is not revealed and we continue by jumping to the next 
                                                # function part

        j       clear_surroundings__epilogue    # otherwise, we jump to the epilogue

clear_surroundings_reveal:

        # Reveal the cell
        # grid[row][col] |= IS_RVLD_MASK;
        or      $s0, $s0, IS_RVLD_MASK          # reveal the cell by doing an "or" bit operation with the value of the
                                                # cell and IS_RVLD_MASK
        sb      $s0, ($t2)                      # and save the new value of the cell

        lw      $t4, cells_left                 # load the cells count left into $t4, decrement it and save it
        addi    $t4, $t4, -1
        sw      $t4, cells_left

        # first check if the cell is marked
        # grid[row][col] &= ~IS_MRKD_MASK
        li      $t7, IS_MRKD_MASK               # unmark the cell by doing an "and" bit operation with the value
        not     $t7, $t7                        # of the cell and the inverse of IS_MRKD_MASK
        and     $s0, $s0, $t7
        
        sb      $s0, ($t2)                      # and save the new value of the cell                           


clear_surroundings_check_value:

        # if (grid[row][col] & VALUE_MASK)      
        and     $t7, $s0, VALUE_MASK                    # check if the cell's last 4 bits have value 0 by doing an
        beqz    $t7, clear_surroundings_recursion       # "and" bit operation and compare the result to 0;
                                                        # jump to the recursion part if the value is 0

        j       clear_surroundings__epilogue            # jump to the epilogue if the value is not 0

clear_surroundings_recursion:
        
        move    $a0, $s1                        # clear_surroundings(row - 1, col)
        move    $a1, $s2
        addi    $a0, $a0, -1
        jal     clear_surroundings

        move    $a0, $s1                        # clear_surroundings(row - 1, col - 1)
        move    $a1, $s2
        addi    $a0, $a0, -1            
        addi    $a1, $a1, -1
        jal     clear_surroundings
        ...
        move    $a0, $s1                        # clear_surroundings(row, col - 1)
        move    $a1, $s2
        addi    $a1, $a1, -1
        jal     clear_surroundings
...
clear_surroundings__epilogue:
        lw      $s0, 12($sp)                    # load the save arguments and the return address from the stack pointer
        lw      $s1, 8($sp)
        lw      $s2, 4($sp)
        lw      $ra, 0($sp)
        addiu   $sp, $sp, 16
